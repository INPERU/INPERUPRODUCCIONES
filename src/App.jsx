...
export default function App() {
  ...
  const [customItemName, setCustomItemName] = useState(''); // NUEVO: Nombre del item manual
  const [quantity, setQuantity] = useState(''); // CAMBIO: Inicia vacío
  const [unitPrice, setUnitPrice] = useState(''); // CAMBIO: Inicia vacío
  ...
  useEffect(() => { if (!user) return; const unsubOrders = onSnapshot(query(collection(db, 'artifacts', appId, 'public', 'data', 'orders')), (s) => { const list = s.docs.map(d => ({ id: d.id, ...d.data() })); list.sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)); setOrders(list); }); const unsubProducts = onSnapshot(query(collection(db, 'artifacts', appId, 'public', 'data', 'products')), (s) => { const list = s.docs.map(d => ({ id: d.id, ...d.data() })); list.sort((a, b) => a.name.localeCompare(b.name)); setProducts(list); }); return () => { unsubOrders(); unsubProducts(); }; }, [user]);
  ...
  const handleSaveOrder = async () => {
    ...
    const orderData = {
      ...
      orderId: newOrderId, 
      // Descripción más limpia para la lista, uniendo con un separador especial
  ...
  const handleEditOrder = (order) => {
    setEditingOrderId(order.id); setNewOrderId(order.orderId); setNewOrderName(order.clientName); setNewOrderPhone(order.phone || ''); setNewOrderSocial(order.social || ''); setNewOrderDeposit(order.deposit || ''); setOrderItems(order.items || []); 
    window.scrollTo({ top: 0, behavior: 'smooth' }); showNotification("Editando pedido #" + order.orderId, "info");
  };
  const handleCancelOrderEdit = () => { setEditingOrderId(null); setNewOrderId(''); setNewOrderName(''); setNewOrderPhone(''); setNewOrderSocial(''); setNewOrderDeposit(''); setOrderItems([]); setCustomDescription(''); setQuantity(''); setCustomItemName(''); };
  ...
  const handleSaveProduct = async () => {
    ...
    const productData = { name: prodName, description: prodDescription, customOptions: prodCustomOptions, price: parseFloat(prodPrice), imageUrl: mainImageUrl, imageUrls: imageUrls, seller: prodSeller, category: 'Papelería', updatedAt: serverTimestamp() };
    try {
  ...
  const sendCartToWhatsapp = () => { if (cart.length === 0) return; if (!clientNameForCart.trim()) { showNotification("Por favor, ingresa tu nombre", "error"); return; } const sellers = new Set(cart.map(item => item.seller || 'dani')); let targetPhone = LINKS.dani; if (sellers.size === 1 && sellers.has('ceci')) { targetPhone = LINKS.ceci; } let message = `Hola INPERU PRODUCCIONES! Soy *${clientNameForCart}*.\nQuiero hacer el siguiente pedido:\n\n`; cart.forEach(item => { message += `▪️ ${item.qty} x ${item.name}`; if (item.selectedOptions) message += `\n   L ${item.selectedOptions}`; message += `\n   L $${(item.price * item.qty).toLocaleString()}\n`; }); message += `\n*Total Estimado: $${cartTotal.toLocaleString()}*\n\nQuedo a la espera asi avanzamos con el pedido. Gracias!`; window.open(`https://wa.me/${targetPhone}?text=${encodeURIComponent(message)}`, '_blank'); setCart([]); setIsCartOpen(false); setClientNameForCart(''); };
          ...
          <div className="animate-in slide-in-from-bottom-4 duration-500 pb-12">
            ...
            {foundOrders.map((order) => {
                ...
                <div key={order.id} className="bg-white rounded-2xl shadow-lg overflow-hidden border border-slate-100 mb-8">
                  ...
                  <div className="p-6">
                    {order.finishedImage && (<div className="mb-6 rounded-xl overflow-hidden border-2 border-teal-500 shadow-lg relative group cursor-pointer" onClick={() => setEnlargedImage(order.finishedImage)}><div className="absolute top-0 left-0 bg-teal-500 text-white text-xs font-bold px-3 py-1 rounded-br-lg z-10">¡Tu Pedido Listo! ✨</div><img src={order.finishedImage} alt="Producto Terminado" className="w-full h-64 object-cover transition transform group-hover:scale-105"/><div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition bg-black/10"><div className="bg-white/90 text-slate-800 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 shadow-sm backdrop-blur-sm"><Maximize2 size={14}/> Ver Foto</div></div></div>)}
                    <div className="flex items-center gap-4 mb-6 p-4 bg-slate-50 rounded-xl border border-slate-100"><div className="bg-teal-50 p-3 rounded-lg"><ShoppingBag className="text-teal-600 w-6 h-6"/></div><div className="flex-1"><p className="text-sm text-slate-500 mb-1">Detalle</p>
                    {/* RENDERIZADO DE DESCRIPCION EN LISTA */}
                    <div className="text-slate-800 font-medium text-sm">
                       {order.description ? order.description.split(' + ').map((line, i) => (
                         <div key={i} className="border-b border-slate-100 last:border-0 py-1">{line}</div>
                       )) : "Sin detalle"}
                    </div>
                    </div></div>
                 ...
                 <div className="bg-white p-6 rounded-xl shadow-lg mb-8 border border-teal-100">
                    ...
                    <div className="bg-slate-50 p-4 rounded-lg mb-4 border border-slate-200">
                       <div className="flex gap-2 mb-2">
                          ...
                          <input type="number" className="p-2 bg-white border border-gray-300 rounded-lg w-16 text-gray-900 text-center" placeholder="Cant" value={quantity} onChange={(e)=>setQuantity(e.target.value)}/>
                       ...
                       {orderItems.map(i => (<div key={i.id} className="flex justify-between text-sm border-b py-1 text-gray-800"><span>{i.quantity} x {i.name}</span><div className="flex gap-4"><span>${i.subtotal}</span><button onClick={()=>handleRemoveItem(i.id)} className="text-red-500">x</button></div></div>))}
                       <div className="text-right font-bold mt-2 text-lg text-gray-900">Total: ${calculateGrandTotal()}</div>
                    </div>

                    <div className="flex justify-end items-center gap-4"><div className="text-right"><span className="text-xs text-slate-400">Seña:</span><input type="number" className="ml-2 p-2 border bg-white border-gray-300 rounded w-24 font-bold text-green-600" value={newOrderDeposit} onChange={(e)=>setNewOrderDeposit(e.target.value)}/></div>{editingOrderId ? (<><button onClick={handleCancelOrderEdit} className="bg-slate-200 text-slate-600 px-6 py-2 rounded-lg font-bold flex items-center gap-2"><RotateCcw size={18}/> Cancelar</button><button onClick={handleSaveOrder} className="bg-teal-600 text-white px-6 py-2 rounded-lg font-bold flex items-center gap-2"><Save size={18}/> Guardar Cambios</button></>) : (<button onClick={handleSaveOrder} className="bg-slate-900 text-white px-6 py-2 rounded-lg font-bold">Crear Pedido</button>)}</div>
                 </div>
                 ...
                 <div className="space-y-4">
                    {orders.map(o => (
                       <div key={o.id} className="bg-white p-4 rounded-xl border shadow-sm relative">
                          ...
                          {/* LISTA DE ITEMS PROLIJA */}
                          <div className="text-sm text-slate-500 mb-2">
                             {o.description && o.description.split(' + ').map((line, i) => (
                               <div key={i} className="border-b border-slate-50 last:border-0 py-1">{line}</div>
                             ))}
                          </div>
                          <div className="flex flex-wrap gap-2 items-center">{/* BOTON EDITAR PEDIDO */}<button onClick={()=>handleEditOrder(o)} className="p-2 rounded-lg bg-slate-100 text-slate-500 hover:bg-teal-50 hover:text-teal-600" title="Editar Pedido"><Pencil size={18}/></button><div className="w-px h-6 bg-slate-200 mx-1"></div>{Object.keys(statusConfig).map(k => k !== 'delivered' && (<button key={k} onClick={()=>updateStatus(o.id, k)} className={`p-2 rounded-lg ${o.status===k ? 'bg-teal-600 text-white':'bg-slate-100 text-slate-400'}`}>{statusConfig[k].label}</button>))} <button onClick={()=>updateStatus(o.id, 'delivered')} className={`p-2 rounded-lg ${o.status==='delivered'?'bg-slate-800 text-white':'bg-slate-100'}`}>Entregar</button><div className="w-px h-6 bg-slate-200 mx-1"></div><button onClick={()=>deleteFinishedPhoto(o)} className={`p-2 rounded-lg border ${o.finishedImage ? 'bg-red-50 text-red-600 border-red-200 hover:bg-red-100' : 'hidden'}`} title="Borrar foto"><Trash2 size={18}/></button><button onClick={()=>addFinishedPhoto(o)} className={`p-2 rounded-lg border ${o.finishedImage ? 'bg-teal-50 text-teal-600 border-teal-200' : 'bg-white text-slate-400 border-slate-200'}`} title="Agregar foto producto terminado"><Camera size={18}/></button><button onClick={()=>sendWhatsAppMessage(o)} className="bg-green-50 text-green-600 border border-green-200 p-2 rounded-lg hover:bg-green-100" title="Enviar aviso por WhatsApp"><Send size={18}/></button>{((o.totalPrice||0) - (o.deposit||0)) > 0 && <button onClick={()=>markAsPaid(o)} className="bg-yellow-50 text-yellow-600 border border-yellow-200 p-2 rounded-lg" title="Saldar deuda"><Banknote size={18}/></button>}<button onClick={()=>deleteOrder(o.id)} className="text-slate-300 hover:text-red-500 p-2"><Trash2 size={16}/></button></div>
                       </div>
                    ))}
                 </div>
               </div>
